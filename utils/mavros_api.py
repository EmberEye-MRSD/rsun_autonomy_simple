...
from collision_prevention import CollisionPreventionInterface
from fiducial_tag_interface import FiducialTagInterface

sys.path.append('../../voxl_ros_utils/scripts')
from voxl_qvio_monitor import VIOMonitorInterface
...
class MavrosAPI():
    ...
    def __init__(self):
        ...
        # rospy.Subscriber('/collision/status',Bool,self.collision_cb)

        self.flag_wp_ready = False # Flag to indicate successful parsing of the .txt file

        # Load external interfaces

        # Collision Avoidance Interface
        self.cp_obj = CollisionPreventionInterface()

        #Configure CPInterface for wall-follow mode
        if self.flag_en_wall_follow:
            self.cp_obj.flag_en_wall_follow = True

        # Precision Landing Interface
        self.pl_obj = FiducialTagInterface()

        if self.flag_en_offset:
            self.pl_obj.set_fixed_frame("map_offset")
        else:
            self.pl_obj.set_fixed_frame("map")

        # VIO Monitor Interface
        self.vio_monitor_obj = VIOMonitorInterface()

        
        # Async loops for collision and qvio monitor interfaces
        rospy.Timer(rospy.Duration(0.10), self.async_loop_cb)        
    ...
    def set_avoidance_params(self, flag):
        self.flag_en_avoid = flag
        self.cp_obj.toggle_avoidance(flag)
    ...
    def do_takeoff(self):
        if self.set_mode('AUTO.TAKEOFF'):
            ...
            self.cp_obj.toggle_avoidance(True)
            return True
    ...
    def publish_setpoints(self, pose_vel_sp):
        # Dynamic avoidance case
        if self.flag_collision and self.flag_en_dyn_avoid:
            rospy.logwarn_throttle(5.0, "[MavrosAPI] MAV in collision proximity! Dynamic avoidance active...")
            # When using dynamic avoidance, replace incoming sp with sp from avoidance logic 
            pose_vel_sp = self.get_avoidance_cmd()
        
        # Passive avoidance
        elif self.flag_collision and not self.flag_en_dyn_avoid and not self.flag_en_wall_follow:
            rospy.logwarn_throttle(5.0, "[MavrosAPI] MAV in collision proximity! Emergency stop active...") 
            # when not using dynamic avoidance, overwrite incoming setpoint
            # to hold current position instead
            pose_vel_sp =PositionTarget()
            pose_vel_sp.header.frame_id = 'map'
            pose_vel_sp.header.stamp = rospy.Time.now()
            pose_vel_sp.coordinate_frame = pose_vel_sp.FRAME_LOCAL_NED
            pose_vel_sp.type_mask = self.POSE_SP_MASK
            pose_vel_sp.position.x = self.obs_det_pos[0]
            pose_vel_sp.position.y = self.obs_det_pos[1]
            pose_vel_sp.position.z = self.obs_det_pos[2]
            pose_vel_sp.yaw = self.curr_yaw
        
        # Wall following mode
        elif self.flag_en_wall_follow and self.curr_api_state==self.api_states.STATE_MISSION_ACTIVE:
            rospy.logwarn_throttle(5.0, "[MavrosAPI] Auto Align Enabled...") 
    
            pose_vel_sp.header.frame_id = 'map'
            pose_vel_sp.header.stamp = rospy.Time.now()
            pose_vel_sp.coordinate_frame = pose_vel_sp.FRAME_LOCAL_NED
            pose_vel_sp.type_mask = self.VEL_YAW_SP_MASK

            avoidance_cmd = self.get_avoidance_cmd()
            wp_cmd = pose_vel_sp.velocity

            # Merge waypoint velocity commands and avoidance velocity commands
            pose_vel_sp.velocity.x = 0.0*avoidance_cmd.velocity.x + wp_cmd.x
            # use only y-direction setpoint
            pose_vel_sp.velocity.y = avoidance_cmd.velocity.y + wp_cmd.y
            pose_vel_sp.velocity.z = 0.0 + wp_cmd.z
    ...
    def async_loop_cb(self, event):

        self.update_collision_status()
        self.update_qvio_status()


    def update_collision_status(self):
        if not self.cp_obj.flag_scan_timeout and self.curr_api_state==self.api_states.STATE_MISSION_ACTIVE:
            flag_in_collision = self.cp_obj.get_collision_status()
            if  flag_in_collision and not self.flag_collision:
                self.obs_det_pos = self.from_pose_msg(self.curr_mav_pose.pose)[0:3]
            
            self.flag_collision = flag_in_collision
        else:
            self.flag_collision = False
    
    def update_qvio_status(self):
        if not self.flag_vio_failed:
            if self.vio_monitor_obj.check_vio_failure():
                self.flag_vio_failed = True
                self.prev_vio_fail_stamp = rospy.Time.now()
            else:
                self.flag_vio_failed = False
        else:
            if (rospy.Time.now()-self.prev_vio_fail_stamp)>rospy.Duration(self.vio_failure_timeout):
                if self.vio_monitor_obj.check_vio_failure():
                    self.flag_vio_failed = True
                    self.prev_vio_fail_stamp = rospy.Time.now()
                else:
        ...
        if (self.flag_vio_failed and self.flag_en_vio_failsafe 
            and (self.curr_api_state==self.api_states.STATE_MISSION_READY or 
                 self.curr_api_state==self.api_states.STATE_MISSION_ACTIVE)):
            
            if self.curr_mav_state.mode!="AUTO.LAND":
                rospy.logwarn_throttle(5.0, "[MavrosAPI] VIO Failure! Change to LAND mode!")
    ...
    def get_avoidance_cmd(self):
        ...
        # Get the latest evasion command from the avoidance interface
        evade_vec = self.cp_obj.get_evade_cmd()
        evade_vec_3d = [evade_vec[0], evade_vec[1], 0.0]

        if not np.isnan(np.linalg.norm(evade_vec)):
            # convert evasion velocity command to Fixed Frame
            evade_vec_ff = np.matmul(evade_vec_3d, euler_matrix(-self.curr_yaw, 0.0, 0.0,'szyx')[:3][:3]) 

            vel_sp = PositionTarget()
            vel_sp.header.frame_id = 'map'
            vel_sp.coordinate_frame = vel_sp.FRAME_LOCAL_NED
            vel_sp.type_mask = self.VEL_ONLY_SP_MASK

            vel_sp.velocity.x = self.dyn_avoid_gain*evade_vec_ff[0]
            vel_sp.velocity.y = self.dyn_avoid_gain*evade_vec_ff[1]
            vel_sp.velocity.z = 0.0
    ...
    # Precision landing routine using Offboard
    def offb_prec_land(self):

        self.set_api_state(self.api_states.STATE_LANDING)

        loop_hz = 20.0
        
        # Check tag data timeout 
        if not self.pl_obj.flag_data_recvd or self.pl_obj.flag_data_timeout:
            rospy.logerr("[MavrosAPI] Invalid landing fiducial tag pose!")
            rospy.logwarn("[MavrosAPI] Falling over to simple landing...")
            self.do_land()
            return
        
        rate = rospy.Rate(loop_hz)

        rospy.loginfo("[MavrosAPI] PrecLand: Starting horizontal approach!")
        tag_pose = self.from_pose_msg(self.pl_obj.get_tag_pose().pose)

        # Horizontal approach
        while np.linalg.norm(self.curr_pos[:2]-tag_pose[:2])>0.25:
            ...
            tag_pose[0] += self.land_offset_x
            tag_pose[1] += self.land_offset_y

            sp_pose = PositionTarget()
            sp_pose.header.stamp = rospy.Time.now()
            sp_pose.header.frame_id = 'map'
            sp_pose.coordinate_frame = sp_pose.FRAME_LOCAL_NED
            sp_pose.type_mask = self.POSE_SP_MASK
            sp_pose.position.x = tag_pose[0]
            sp_pose.position.y = tag_pose[1]
            sp_pose.position.z = self.takeoff_alt
            sp_pose.yaw = self.curr_yaw

            self.publish_setpoints(sp_pose)
            rate.sleep()

        self.offb_hold([self.curr_pos[0], self.curr_pos[1], self.curr_pos[2], self.curr_yaw], 2.0)

        # Align Yaw
        while abs(self.curr_yaw-tag_pose[3])>0.10:
            ...
            sp_pose.position.y = tag_pose[1]
            sp_pose.position.z = self.takeoff_alt
            sp_pose.yaw = tag_pose[3]

            self.publish_setpoints(sp_pose)
            rate.sleep()
        
        rospy.loginfo("[MavrosAPI] PrecLand: Aligned with Tag! Holding position briefly...")
        
        # Hold position briefly to stabilise the drone
        self.offb_hold([self.curr_pos[0], self.curr_pos[1], self.curr_pos[2], self.curr_yaw], 5.0)
        
        rospy.loginfo("[MavrosAPI] PrecLand: Starting precision descend...")

        # Start descent
        curr_alt_sp = self.curr_pos[2]
        alt_decrement = self.land_speed/loop_hz

        # Precision descent phase
        while self.curr_pos[2]>self.prec_land_final_alt:
            ...
            sp_msg.type_mask = self.POSE_SP_MASK
            sp_msg.header.frame_id="map"
            sp_msg.header.stamp = rospy.Time.now()
            sp_msg.coordinate_frame = sp_msg.FRAME_LOCAL_NED
            sp_msg.position.x = tag_pose[0]
            sp_msg.position.y = tag_pose[1]
            sp_msg.position.z = curr_alt_sp
            sp_msg.yaw = tag_pose[3]
            # TODO: Try adding velocity feed-forward as well
            # sp_msg.velocity.x = 1.75*(tag_pose[0]-self.curr_pos[0])
            # sp_msg.velocity.y = 1.75*(tag_pose[1]-self.curr_pos[1])
            # sp_msg.velocity.z = -0.5*self.land_speed
            curr_alt_sp = curr_alt_sp - alt_decrement

            self.publish_setpoints(sp_msg)
            rate.sleep()
        
        rospy.loginfo("[MavrosAPI] PrecLand: Reached critical altitude! Starting normal landing...")
        self.do_land()
